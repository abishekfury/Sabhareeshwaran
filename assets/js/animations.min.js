...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function initHeroSequenceAnimation() {
    ...
    function render(index) {
      ...
      // Responsive framing & alignment across device viewports
      if (window.innerWidth <= 768) {
        // Mobile Alignment:
        // Frame Sab cleanly in the upper ~54% of the screen so head & sunglasses are 100% visible,
        // leaving the lower section for subtitle, buttons, and SAB headline.
        const targetH = Math.min(ch * 0.54, Math.max(ch * 0.44, cw * 1.05));
        const mobileRatio = targetH / imgH;
        drawW = Math.round(imgW * mobileRatio);
        drawH = Math.round(imgH * mobileRatio);
        offsetX = Math.round((cw - drawW) * 0.5);
        offsetY = Math.round(Math.max(14 * (window.devicePixelRatio || 1), ch * 0.04));
      } else if (window.innerWidth <= 1024) {
        // Tablet Alignment:
        const ratio = Math.max(cw / imgW, ch / imgH);
        drawW = imgW * ratio;
        drawH = imgH * ratio;
        offsetX = (cw - drawW) * 0.52;
        offsetY = (ch - drawH) * 0.15;
      } else {
        // Desktop Alignment:
        const ratio = Math.max(cw / imgW, ch / imgH);
        drawW = imgW * ratio;
        drawH = imgH * ratio;
        offsetX = (cw - drawW) * 0.54;
        offsetY = (ch - drawH) * 0.2;
      }
    ...
    // High-performance on-demand frame loader
    function requestFrame(idx) {
      if (idx < 0 || idx >= totalFrames || images[idx]) return;
      const img = new Image();
      img.decoding = 'async';
      img.src = framePaths[idx];
      img.onload = () => {
        images[idx] = img;
        if (currentFrame.index === idx) {
          render(idx);
        }
      };
      img.onerror = () => {
    ...
    function requestFrameRange(centerIdx, ahead = 12, behind = 6) {
      const start = Math.max(0, centerIdx - behind);
      const end = Math.min(totalFrames - 1, centerIdx + ahead);
      for (let i = start; i <= end; i++) {
        requestFrame(i);
      }
    }

    // Preload images progressively & non-blocking
    function preloadImages() {
      // Step 1: Load Frame 0 (ezgif-frame-001) immediately
      ...
      // Step 2: Request first 8 frames so immediate scroll starts silky smooth
      requestFrameRange(0, 8, 0);

      // Step 3: Defer sparse milestone warmup & background trickle until page is idle after load
      function idleWarmup() {
        const milestones = [24, 48, 72, 96, 120, 144, 168, 192, 216, 235];
        milestones.forEach((idx) => requestFrame(idx));

        let trickleIdx = 1;
        function trickleRest() {
          if (trickleIdx >= totalFrames) return;
          for (let k = 0; k < 2 && trickleIdx < totalFrames; k++, trickleIdx++) {
            requestFrame(trickleIdx);
          }
          if ('requestIdleCallback' in window) {
            window.requestIdleCallback(trickleRest, { timeout: 1000 });
          } else {
            setTimeout(trickleRest, 200);
          }
        }
        if ('requestIdleCallback' in window) {
          window.requestIdleCallback(trickleRest, { timeout: 2000 });
        } else {
          setTimeout(trickleRest, 500);
        }
      }
      ...
      if (document.readyState === 'complete') {
        ...
        window.addEventListener('load', () => setTimeout(idleWarmup, 1500), { once: true });
      }
    }
    ...
    if (hudButtons && hudButtons.length) {
      hudButtons.forEach((btn) => {
        btn.addEventListener('click', (e) => {
          ...
          requestFrameRange(targetIdx, 10, 10);
    ...
    const heroTimeline = gsap.timeline({
      scrollTrigger: {
        ...
        onUpdate: (self) => {
          ...
          requestFrameRange(frameIndex, 12, 6);
  ...
  function initCustomCursor() {
    ...
    gsap.ticker.add(() => {
      if (dot.parentElement !== document.body) document.body.appendChild(dot);
    ...
    setupCursorHover('a, button, [role="button"], .clickable, .nav-menu-btn, .rec-btn, .mobile-menu-close, .menu-option-link, .page-wrapper-close, .nav-drawer-close, .nav-cta, .nav-logo, .theme-toggle-btn', 'hover-link');
